...
function Section3() {
  ...
  useEffect(() => {
    fetch(
      "https://api.themoviedb.org/3/discover/movie?api_key=1a05bd3a661c3ad18b28dfdde27416e8&with_keywords=197345",
      options
    )
      .then((res) => res.json())
  ...
  useEffect(() => {
    if (movie.length === 0) {
      ...
      dispatch(cartoon5(movie));
    }
  }, [movie]);

  // get all movies from redux store
  const trandingPage1 = useSelector((state) => state.cartoon5);
